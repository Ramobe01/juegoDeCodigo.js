...
function generarMapa(){
    let xTesoro = Math.floor(Math.random()*5)+1
    let yTesoro = Math.floor(Math.random()*5)+1
    tesoro.push(xTesoro,yTesoro)
    let xUsuario = Math.floor(Math.random()*5)+1
    let yUsuario = Math.floor(Math.random()*5)+1
        ...
        while ((usuario.vida > 0 && pirata.vida > 0)) {
            ...
            if (pirata.vida <= usuario.ataque && pirata.vida > 0) {
                ...
                switch (continuarPeleando.toUpperCase()) {
                    ...
                    pirata.vida -= usuario.ataque;
                    console.log("------------------------------------------------------------------")
                    console.log(`%c¡HAS GANADO EL DUELO!\n${pirata.nombre.toUpperCase()} TENIA EL TESORO. %c¡LO HAS ENCONTRADO!`,"color:rgb(189,255,174)","color:rgb(230, 179, 27)");
                    inputUsuario = false
                    break;
